...
NuDEXLevelDensity::NuDEXLevelDensity(int aZ,int aA,int ldtype){
...
int NuDEXLevelDensity::ReadLDParameters(const char* dirname,const char* inputfname,const char* defaultinputfname){

  char fname[100];
  if(LDType==1 || LDType==3){ // Back-Shifted-Fermi-Gas model
    sprintf(fname,"%s/LevelDensities/level-densities-bfmeff.dat",dirname);
  }
  else{ //  Constant Temperature
    sprintf(fname,"%s/LevelDensities/level-densities-ctmeff.dat",dirname);
  }
  double EL=0,EU=0;
  ...
  if(!HasData){ //no data
    int check=CalculateLDParameters_BSFG(dirname);
    if(check==0){
      HasData=true;
      if(LDType==2){
	LDType=1;
...
int NuDEXLevelDensity::CalculateLDParameters_BSFG(const char* dirname){

  //Eq. 61 of RIPL-3:
  double alpha=0.0722396; //MeV^{-1}
  double beta= 0.195267; //MeV^{-1}
  double gamma0=0.410289; //MeV^{-1}
  double delta=0.173015; //MeV
  ...
  Delta_ldpar=n_par*12/sqrt(A_mass)+delta;

  //ainf_ldpar: Eq. 52 of RIPL-3:
  ainf_ldpar=alpha*A_Int+beta*pow(A_mass,2./3.);

  //gamma_ldpar: Eq. 53 of RIPL-3:
  gamma_ldpar=gamma0/pow(A_mass,1./3.);

  //dW_ldpar --> from data file
  char fname[100];
  sprintf(fname,"%s/LevelDensities/shellcor-ms.dat",dirname);
  std::ifstream in(fname);
  if(!in.good()){
    std::cout<<" ######## Error opening file "<<fname<<" ########"<<std::endl;
    NuDEXException(__FILE__,std::to_string(__LINE__).c_str(),"##### Error in NuDEX #####");
...
int NuDEXLevelDensity::SearchLDParametersInInputFile(const char* inputfname){
...
void NuDEXLevelDensity::PrintParametersInInputFileFormat(std::ostream &out){
...
double NuDEXLevelDensity::GetNucleusTemperature(double ExcEnergy){
  ...
  double Uval=ExcEnergy-Delta_ldpar;
  if(Uval<=0){return 0;}
  double a_ldpar=ainf_ldpar*(1.+dW_ldpar/Uval*(1.-exp(-gamma_ldpar*Uval)));
  if(LDType==3){
    a_ldpar=ainf_ldpar;
  }
  return sqrt(Uval/a_ldpar);
...
//Gilbert-Cameron:
double NuDEXLevelDensity::GetLevelDensity(double ExcEnergy,double spin,bool ,bool TotalLevelDensity){
  ...
  if(((A_Int+(int)(spin*2+0.01))%2)!=0 && (TotalLevelDensity==false)){
    return 0;
  }
  ...
  if(LDType==3){
    double sig2=0.0888*pow(A_mass,2./3.)*sqrt(ainf_ldpar*Uval);
    double sig=sqrt(sig2);
    double rho=0.05893*exp(2.*sqrt(ainf_ldpar*Uval))/sig/pow(ainf_ldpar,0.25)/pow(Uval,1.25);
    double xj2=(spin+0.5)*(spin+0.5);
    double fj=(2.*spin+1.)/2./sig2*exp(-xj2/2./sig2);
    return 0.5*fj*rho;
  }
  //----------------------------------------------------------------
  ...
  double Uval_Sn=Sn-Delta_ldpar;
  double a_ldpar=ainf_ldpar*(1.+dW_ldpar/Uval*(1.-exp(-gamma_ldpar*Uval)));
  double a_ldpar_Sn=ainf_ldpar*(1.+dW_ldpar/Uval_Sn*(1.-exp(-gamma_ldpar*Uval_Sn)));
  double sigma2_f=0.01389*pow(A_mass,5./3.)/ainf_ldpar*sqrt(a_ldpar*Uval);
  double sigma2_f_Sn=0.01389*pow(A_mass,5./3.)/ainf_ldpar*sqrt(a_ldpar_Sn*Uval);
  double sigma2_d=(0.83*pow(A_mass,0.26))*(0.83*pow(A_mass,0.26));
  ...
  double statfactor=1./2.*(2.*spin+1.)/(2.*sigma2)*exp(-(spin+1/2.)*(spin+1/2.)/2./sigma2);
  ...
  if(LDType==2 && ExcEnergy<Ex_ldpar){
    double totalrho=exp((ExcEnergy-E0_ldpar)/T_ldpar)/T_ldpar;
    return totalrho*statfactor;
  }

  //Else: BSFGM (LDType==1 or ExcEnergy>Ex_ldpar)
  double rhotot_f=1./sqrt(2.*sigma2)/12.*exp(2.*sqrt(a_ldpar*Uval))/pow(a_ldpar,1./4.)/pow(Uval,5./4.);
  double rhotot_0=exp(1.)*a_ldpar/12./sqrt(sigma2)*exp(a_ldpar*Uval);
  double totalrho=1./(1./rhotot_f+1./rhotot_0);
...
double NuDEXLevelDensity::EstimateInverse(double LevDen_iMeV,double spin,bool parity){

  //We assume that rho is a monotonically increasing function

  double tolerance=0.001; //the result will have this relative tolerance. 0.01 means 1%
...
double NuDEXLevelDensity::Integrate(double Emin,double Emax,double spin,bool parity){

  int nb=1000;
  double Integral=0,x1,x2,y1,y2;
  for(int i=0;i<nb;i++){
    x1=Emin+(Emax-Emin)*i/(double)(nb-1.);
    x2=Emin+(Emax-Emin)*(i+1.)/(double)(nb-1.);
...
void NuDEXLevelDensity::PrintParameters(std::ostream &out){
